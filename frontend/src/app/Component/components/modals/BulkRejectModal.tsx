...
export default function BulkRejectModal({ open, count, onClose, onReject, isBusy }: Props) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30" role="dialog" aria-modal="true">
      <div className="dialog-surface-login w-full max-w-lg">
        <div className="px-5 py-3 border-b dialog-divider-login flex items-center justify-between">
          <h3 className="text-lg font-semibold text-slate-800">Reject {count} Requests</h3>
        ...
        <div className="p-5 space-y-3">
          <div className="text-sm">Provide a reason applied to all selected requests.</div>
          <div>
        ...
        <div className="px-5 py-3 border-t dialog-divider-login flex items-center justify-end gap-2">
          ...
          <button onClick={submit} disabled={isBusy} className="rounded-md bg-rose-600 text-white px-4 py-1.5 text-sm disabled:opacity-50 cursor-pointer">
            {isBusy ? "Rejecting…" : "Reject Selected"}
          </button>
        </div>
      </div>
    </div>
